...
from moviepy import TextClip, CompositeVideoClip, AudioFileClip, VideoFileClip, CompositeAudioClip, ImageClip, afx, concatenate_videoclips
...
def edit(subtitles, chunked_text, music_path, background_path, color1, char_name1, color2=None, char_name2=None, has_images=False, has_character=False, character_apper_durations=None, gen_type='monologue'):
    ...
    duration = audio.duration + 1  # 1s grace after TTS ends

    raw_bg = VideoFileClip(f"static/background_videos/{background_path}").resized((1080, 1920))
    if raw_bg.duration < duration:
        loops_needed = math.ceil(duration / raw_bg.duration)
        raw_bg = concatenate_videoclips([raw_bg] * loops_needed)
    background = raw_bg.subclipped(0, duration)

    if music_path != 'no-music':
        raw_music = AudioFileClip(f'static/musics/{music_path}')
        if raw_music.duration < duration:
            loops_needed = math.ceil(duration / raw_music.duration)
            raw_music = concatenate_videoclips([raw_music] * loops_needed)
        bg_music = raw_music.subclipped(0, duration)
    ...
    final.write_videofile(
        f'static/temp/videos/video{filename}.mp4',
        fps=24,
        threads=4,
        preset='ultrafast',
    )